...
// 获取账户管理数据列表
router.get('/account-management', authMiddleware(true), requirePerm('view_account_management'), async (req, res) => {
  try {
    const { page = 1, pageSize = 20, sort = 'id', order = 'desc' } = req.query
    const offset = (Number(page) - 1) * Number(pageSize)
    
    // 获取总数
    const total = await query('select count(*) from account_management')
    ...
    const result = await query(
      `select 
        ...
        cheque_ref_no as "chequeRefNo",
        debit_amount as "debitAmount",
        credit_amount as "creditAmount",
        reference_1 as "reference1",
        reference_2 as "reference2",
        reference_3 as "reference3"
    ...
    console.error('获取账户管理数据失败:', err)
    res.status(500).json({ error: '获取数据失败', detail: err.message })
  }
})

// 导入账户管理数据
router.post('/account-management/import', authMiddleware(true), requirePerm('view_account_management'), async (req, res) => {
  try {
    const { rows } = req.body || {}
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // 检查必填字段
      if (!r.accountNumber || !r.transactionDate) {
      ...
      try {
        // 转换日期格式，确保是有效的日期
        let transactionDate = r.transactionDate
        if (!/^\d{4}-\d{2}-\d{2}$/.test(transactionDate)) {
          // 尝试解析其他日期格式并标准化为 YYYY-MM-DD
        ...
        // 有效数据添加到待插入列表
        valids.push({
          account_number: String(r.accountNumber).trim(),
          transaction_date: transactionDate,
          cheque_ref_no: r.chequeRefNo ? String(r.chequeRefNo).trim() : null,
          debit_amount: parseNum(r.debitAmount),
          credit_amount: parseNum(r.creditAmount),
          reference_1: r.reference1 ? String(r.reference1).trim() : null,
          reference_2: r.reference2 ? String(r.reference2).trim() : null,
          reference_3: r.reference3 ? String(r.reference3).trim() : null
        })
      } catch (e) {
    ...
    // 批量插入有效数据
    let inserted = 0
    for (const record of valids) {
      try {
        await query(
          `insert into account_management(
            account_number, transaction_date, cheque_ref_no, 
            debit_amount, credit_amount, 
            reference_1, reference_2, reference_3
          ) values($1, $2, $3, $4, $5, $6, $7, $8)
          on conflict (account_number, transaction_date) do update set
            cheque_ref_no = $3,
            debit_amount = $4,
            credit_amount = $5,
            reference_1 = $6,
            ...
            updated_at = now()`,
          [
            record.account_number, record.transaction_date, record.cheque_ref_no,
            record.debit_amount, record.credit_amount,
            record.reference_1, record.reference_2, record.reference_3
          ]
        )
        inserted++
      } catch (e) {
        errors.push({ 
          record: record.account_number + ' / ' + record.transaction_date, 
          error: e.message 
        })
      }
    }
