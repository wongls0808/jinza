  ...
  <div class="suppliers">
    ...
    <div v-else>
      <div v-if="!suppliers.length" class="empty-state">
        ...
        <el-button type="primary" link @click="showAddDialog = true">+ 新增供应商</el-button>
      </div>
      <div v-else class="card-grid">
        <div class="supplier-card data-card" v-for="s in suppliers" :key="s.id">
          <div class="card-top">
            <div class="logo-circle">{{ (s.name || '?').charAt(0).toUpperCase() }}</div>
            <div class="meta">
              <div class="name-row">
              ...
              <div class="line" v-if="s.contact" :title="s.contact">联系人：{{ s.contact }}</div>
              <div class="line" v-if="s.registration_no" :title="s.registration_no">注册号：{{ s.registration_no }}</div>
              <div class="line" v-if="s.tax_no" :title="s.tax_no">税号：{{ s.tax_no }}</div>
              <div class="line" v-if="s.phone" :title="s.phone">联系电话：{{ s.phone }}</div>
              <div class="line email" v-if="s.email" :title="s.email">邮箱：{{ s.email }}</div>
              <div class="line" v-if="s.address" :title="s.address">地址：{{ s.address }}</div>
              <div class="tags-row" v-if="s.tags && s.tags.length">
                <el-tag
                  v-for="(t,i) in s.tags.slice(0,3)"
                  :key="t+i"
                  size="small"
                  class="tag-item dynamic-tag"
                  :style="tagStyle(t)"
                >{{ t }}</el-tag>
                <el-tag v-if="s.tags.length>3" size="small" effect="plain">+{{ s.tags.length-3 }}</el-tag>
              </div>
            </div>
            <div class="actions">
              <el-dropdown trigger="click" @command="cmd => handleCardCommand(cmd, s)">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item command="toggle">{{ s.status === 'active' ? '停用' : '启用' }}</el-dropdown-item>
          ...
          <div class="card-footer">
            <span class="footer-text">状态：{{ s.status === 'active' ? '活跃' : '停用' }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- 新增 / 编辑 供应商对话框 -->
    <el-dialog :title="isEdit ? '编辑供应商' : '新增供应商'" v-model="showAddDialog" width="640px">
...
const submitForm = () => {
  formRef.value.validate(async (valid) => {
    ...
    try {
      if (isEdit.value) {
        const resp = await fetch(`/api/suppliers/${editingId.value}`, {
          method: 'PUT',
...
const handleToggle = async (row) => {
  ...
  try {
    const resp = await fetch(`/api/suppliers/${row.id}/status`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
/* 特有样式 */
.logo-circle { width: 46px; height: 46px; border-radius: 12px; background: linear-gradient(135deg,#409EFF,#1677FF); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 600; font-size: 18px; flex-shrink: 0; }
